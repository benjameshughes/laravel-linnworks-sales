<div class="min-h-screen bg-gradient-to-br from-zinc-50 to-zinc-100 dark:from-zinc-900 dark:to-zinc-950 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {{-- Header --}}
        <div class="mb-8">
            <div class="flex items-center gap-3 mb-2">
                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl shadow-lg">
                    S
                </div>
                <div>
                    <flux:heading size="xl" class="text-zinc-900 dark:text-zinc-100">Sophie's Variation Group Sales</flux:heading>
                    <flux:subheading class="text-zinc-600 dark:text-zinc-400">Complete breakdown by variation groups and subsources</flux:subheading>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-4 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                {{-- Date Range Filter --}}
                <div>
                    <flux:label class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400 mb-2">Date Range</flux:label>
                    ...
                    <thead class="bg-gradient-to-r from-zinc-100 to-zinc-50 dark:from-zinc-900 dark:to-zinc-800 border-b-2 border-zinc-300 dark:border-zinc-600">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left">
                                <button wire:click="sortBy('sku')" class="flex items-center gap-2 font-semibold text-zinc-900 dark:text-zinc-100 hover:text-pink-600 dark:hover:text-pink-400 transition-colors">
                                    SKU
                                    @if($sortBy === 'sku')
                            ...
                            <th class="px-6 py-4 text-right">
                                <button wire:click="sortBy('orders')" class="flex items-center gap-2 font-semibold text-zinc-900 dark:text-zinc-100 hover:text-pink-600 dark:hover:text-pink-400 transition-colors ml-auto">
                                    Orders
                                    @if($sortBy === 'orders')
                                        <flux:icon :name="$sortDirection === 'asc' ? 'arrow-up' : 'arrow-down'" class="size-4" />
                                    @endif
                            ...
                            <th class="px-6 py-4 text-right">
                                <button wire:click="sortBy('units')" class="flex items-center gap-2 font-semibold text-zinc-900 dark:text-zinc-100 hover:text-pink-600 dark:hover:text-pink-400 transition-colors ml-auto">
                                    Units Sold
                                    @if($sortBy === 'units')
                                        <flux:icon :name="$sortDirection === 'asc' ? 'arrow-up' : 'arrow-down'" class="size-4" />
                                    @endif
                            ...
                            <th class="px-6 py-4 text-right">
                                <button wire:click="sortBy('revenue')" class="flex items-center gap-2 font-semibold text-zinc-900 dark:text-zinc-100 hover:text-pink-600 dark:hover:text-pink-400 transition-colors ml-auto">
                                    Revenue
                                    @if($sortBy === 'revenue')
                                        <flux:icon :name="$sortDirection === 'asc' ? 'arrow-up' : 'arrow-down'" class="size-4" />
                                    @endif
                    ...
                    <tbody>
                        @forelse($this->variationGroups as $index => $group)
                            ...
                            <tr class="border-b border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors {{ in_array($group['sku'], $expandedRows) ? 'bg-pink-50 dark:bg-pink-900/10' : '' }}">
                                <td class="px-6 py-4">
                                    <button wire:click="toggleRow('{{ $group['sku'] }}')" class="w-8 h-8 rounded-lg bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold shadow hover:shadow-lg transition-all hover:scale-110">
