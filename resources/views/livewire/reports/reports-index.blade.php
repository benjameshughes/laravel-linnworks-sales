<div class="min-h-screen max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8 flex items-start justify-between">
        ...
        <flux:button variant="ghost" wire:navigate href="/reports/compare">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="border-b border-zinc-200 dark:border-zinc-700 last:border-b-0">
                        ...
                        <div class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            @foreach($reports as $report)
                                ...
                                @endphp
                                <button
                                    wire:click="selectReport('{{ $reportSlug }}')"
                                    class="w-full px-4 py-3 text-left hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors {{ $selectedReportSlug === $reportSlug ? 'bg-pink-50 dark:bg-pink-900/20 border-l-4 border-pink-500' : '' }}"
                                >
                                    <div class="flex items-start gap-3">
                                        <flux:icon :name="$report->icon()" class="size-5 mt-0.5 {{ $selectedReportSlug === $reportSlug ? 'text-pink-600 dark:text-pink-400' : 'text-zinc-500 dark:text-zinc-400' }}" />
                                        <div class="flex-1 min-w-0">
                                            <h4 class="text-sm font-medium {{ $selectedReportSlug === $reportSlug ? 'text-pink-900 dark:text-pink-100' : 'text-zinc-900 dark:text-zinc-100' }}">
                                                {{ $report->name() }}
                                            </h4>
        ...
        <div class="lg:col-span-2">
            @if($selectedReportSlug)
                @livewire('reports.report-viewer', ['reportSlug' => $selectedReportSlug], key($selectedReportSlug))
            @else
